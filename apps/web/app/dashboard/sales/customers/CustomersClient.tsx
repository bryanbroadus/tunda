...
export default function CustomersClient({ initialCustomers, businessId, role }: Props) {
  ...
  const [customerHistory, setCustomerHistory] = useState<{ invoices: InvoiceSummary[]; payments: InvoicePayment[] } | null>(null)
  ...
  async function openCustomer(customer: Customer) {
    ...
    const { data: invoices } = await supabase
      .from('invoices')
      .select('id, invoice_number, created_at, total_amount, amount_paid, status, payment_method')
      .eq('customer_id', customer.id)
      .order('created_at', { ascending: false })
      .limit(20)

    // Fetch payments linked to the customer's invoices
    const invoiceIds = invoices?.map(i => i.id) ?? []
    let paymentsData: InvoicePayment[] = []
    if (invoiceIds.length > 0) {
      const { data: pays } = await supabase
        .from('invoice_payments')
        .select('id, created_at, amount, note, payment_method, payment_date')
    ...
    setCustomerHistory({ invoices: invoices ?? [], payments: paymentsData })
  }
  ...
  async function handleRecordPayment(e: React.FormEvent) {
    ...
    const { data: oldestInvoice, error: invErr } = await supabase
      .from('invoices')
    ...
    if (invErr || !oldestInvoice) {
      setError('No unpaid invoices found for this customer.')
      setSaving(false)
      return
    }
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          onClick={() => { setEditingCustomer(null); setAddForm({ name: '', phone: '', credit_limit: '' }); setError(''); setShowAddForm(true) }}
      ...
      {selectedCustomer && (
        <Modal
          ...
          onClose={() => { setSelectedCustomer(null); setCustomerHistory(null); setShowPaymentForm(false) }}
        >
          <div className="space-y-4">
            ...
            {selectedCustomer.credit_balance > 0 && (
              <>
                {!showPaymentForm ? (
                  <button
                    onClick={() => { setShowPaymentForm(true); setPaymentAmount(''); setPaymentNote('') }}
            ...
            {customerHistory && (
              <div>
                ...
                <div className="max-h-60 overflow-y-auto space-y-1">
                  {[
                    ...customerHistory.invoices.map(i => ({
                      type: 'invoice' as const,
                      date: i.created_at,
                      amount: i.total_amount,
                      label: `${i.invoice_number} (${i.payment_method ?? 'credit'})`,
                      id: i.id,
                    })),
                  ...
                  {customerHistory.invoices.length === 0 && customerHistory.payments.length === 0 && (
